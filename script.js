...
function updateGameInfo() {
    ...
    if (!gameInfoElement) {
        ...
        gameInfoElement.className = 'game-info-display';

        // Insertarlo en la columna central, antes del sistema de dados
        const centralColumn = document.querySelector('.col-central');
        const diceSystem = document.querySelector('.dice-system');
        if (centralColumn && diceSystem) {
            centralColumn.insertBefore(gameInfoElement, diceSystem);
        }
    }
    ...
    gameInfoElement.innerHTML = `
        <div class="game-status">
            ...
            <div class="inning-info">
                <strong>Inning ${gameState.currentInning}</strong> - 
                ${gameState.isTopHalf ? '⬆️ Arriba (Visitante batea)' : '⬇️ Abajo (Local batea)'}
            </div>
            <div class="batter-info">
                <strong>Al bate:</strong> ${batterName} (${battingTeam})
            </div>
            <div class="count-info">
                <span class="outs">Outs: ${gameState.outs}</span> |
                <span class="balls">Bolas: ${gameState.balls}</span> |
                <span class="strikes">Strikes: ${gameState.strikes}</span>
            </div>
        </div>
    `;
}
...
// Función para mostrar resultado de tirada en los contenedores laterales
function displayDiceResult(team, totalResult, description, requiresAdditional = false, additionalTypes = []) {
...
function toggleGameControls() {
    ...
    if (gameState.isGameActive) {
        // Juego activo: ocultar iniciar, mostrar reiniciar
        if (startBtn) startBtn.style.display = 'none';
        if (resetBtn) resetBtn.style.display = 'inline-block';
    } else {
        // Juego inactivo: mostrar iniciar, ocultar reiniciar
